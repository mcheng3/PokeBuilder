...
$( document).ready(function() {
    ...
    $("#pokemon").on("select2:select", function(e) {
        ...
        $("#pokesubmit").attr("class", "btn btn-dark disabled");
...
function fillForm(pkmnid){
    ...
    $.ajax({
        ...
        success: function(d){
            d = JSON.parse(d);
            input = d['name'];
            movelist = d['moves'].split(',');
            var ability = d['ability'];
            console.log(movelist);
            console.log(input);
            console.log("ability: " + ability);
            var index = pokemon.indexOf(input);
            pokemon.splice(index, 1);
            pokemon.unshift(input);
            $("#pokemon").empty();
            $("#pokemon").select2({ 
                data: pokemon
            });
            $.ajax({
               ...
               success: function(d){
                ...
                for(i = 0; i < 4; i++){
                    var index = moves.indexOf(movelist[i]);
                    moves.splice(index, 1);
                    moves.unshift(movelist[i]);
                    $("#moveslist" + i).empty();
                    $("#moveslist" + i).select2({
                        data: moves
                    });
